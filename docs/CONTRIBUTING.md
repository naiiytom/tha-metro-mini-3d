...
- **Camera rotation is a single merged orbit** (`src/map/cameraControls.ts`, which disables MapLibre's `dragRotate` and owns both axes). Middle-, right- and ctrl+left-drag all orbit: vertical pitches, horizontal turns, both at once, in MapLibre's own directions. Pan and scroll-zoom stay MapLibre's. The camera harness that asserted every direction with real mouse events was removed 2026-08-09.
