...
- **Train scale & visibility toggle (GitHub issue #5):** Added `1x` (realistic 1:1), `1.5x` (+50%), `3x`, and `5x` train model scaling in `ViewControls`, allowing users to adjust visibility for overview zooms and mobile screens while preserving track deck alignment, procedural liveries, and pick hitbox bounds.
