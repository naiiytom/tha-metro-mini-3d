 ...
 *
 * Provides configurable train model scaling (1x realistic, 1.5x, 3x, 5x)
 * so users can adjust visibility at overview zooms and on small mobile screens.
 */

export type TrainScale = 1 | 1.5 | 3 | 5;
...
export const TRAIN_SCALES: readonly TrainScaleOption[] = [
  ...
  { scale: 3, label: "3x", hint: "3x scale — prominent visibility across the metropolitan area" },
  { scale: 5, label: "5x", hint: "5x scale — maximum visibility for high altitudes and small screens" },
];
...
export function parseTrainScale(raw: string | null): TrainScale {
  ...
  if (raw === "3") return 3;
  if (raw === "5") return 5;
  // Legacy migration for previously stored 2x scale
  if (raw === "2") return 3;
  return 1;
}
